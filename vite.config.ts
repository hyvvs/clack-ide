...
// Bundle/treemap analysis is opt-in: `ANALYZE=true npm run build` emits stats.html.
const analyze = process.env.ANALYZE === "true";

// Module-graph inspector is opt-in via `npm run dev:inspect`; keeps plain
// `npm run dev` from paying its transform-tracking overhead on every run.
...
export default defineConfig(async ({ mode }): Promise<UserConfig> => ({
  plugins: [
    ...
    // transforms). Opt-in via `npm run dev:inspect`, never in a production build.
