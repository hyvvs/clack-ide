...
Clack is a fork of Terax with a strong product direction. Contributions are welcome, but **alignment matters more than volume**.
...
- Clack is currently moving fast, with Terax kept as the upstream architecture base.
...
Prereqs: Rust (stable), Node 20+, npm, plus your platform's [Tauri prerequisites](https://tauri.app/start/prerequisites/).
...
Clack positions itself as **lightweight, fast, production-grade**. Every PR is reviewed against:
...
The most common way a PR breaks Clack is a **local fix with global blast radius**: the diff solves one reported case, reads fine, passes type-check and clippy, and silently breaks the same subsystem in every other case. Review alone does not catch these. A test does.
...
- Clack is not trying to be a full IDE replacement (VS Code, Cursor, Zed).
