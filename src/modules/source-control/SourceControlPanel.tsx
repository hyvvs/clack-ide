...
const SOURCE_CONTROL_TOOLTIP_CLASS =
  "border border-[color:var(--clack-border-strong)] bg-[var(--clack-surface-raised)] text-[var(--clack-text-1)] shadow-lg shadow-black/30";
...
export const SourceControlPanel = memo(function SourceControlPanel({
  ...
  const commitShortcut = IS_MAC ? "Cmd+Enter" : "Ctrl+Enter";
  const generateShortcut = IS_MAC ? "Cmd+G" : "Ctrl+G";
  const canCommit =
      ...
      >
        <header className="clack-shell flex shrink-0 items-center justify-between gap-2 border-b px-3 py-2.5">
          <div className="flex min-w-0 items-center gap-1.5">
            ...
            {scm.status && (scm.status.ahead > 0 || scm.status.behind > 0) ? (
              <div className="flex shrink-0 items-center gap-0.5 text-[10px] font-semibold tabular-nums leading-none text-[var(--clack-text-3)]">
                {scm.status.ahead > 0 ? (
                  <span className="inline-flex items-center gap-0.5 rounded-[var(--clack-radius-button)] border border-[color:var(--clack-border-subtle)] px-1 py-0.5">
                    <HugeiconsIcon
                      icon={ArrowUp01Icon}
                      size={9}
                      strokeWidth={2.2}
            ...
            {scm.status?.isDetached ? (
              <span className="rounded-[var(--clack-radius-button)] bg-[var(--clack-surface-2)] px-1.5 py-0.5 text-[10px] font-medium uppercase text-[var(--clack-text-3)]">
                detached
              </span>
            ) : null}
          </div>
        ...
        {onOpenGitGraph ? (
          <button
            ...
            className="group flex shrink-0 cursor-pointer items-center gap-2 border-b border-[color:var(--clack-border-subtle)] px-3 py-2 text-left text-[var(--clack-text-3)] transition-colors hover:bg-[rgba(159,177,210,0.07)] hover:text-[var(--clack-text-1)]"
          >
            <HugeiconsIcon
              icon={GitBranchIcon}
              size={13}
        ...
        {scm.panelState === "ready" && scm.status ? (
          <>
            <div className="relative shrink-0 space-y-2 border-b border-[color:var(--clack-border-subtle)] bg-[var(--clack-surface-1)] px-2.5 pb-2.5 pt-2.5">
              <div
                className={cn(
                  "clack-card relative transition-colors",
                  scm.commitMessage.length > 0
                    ? "border-[color:var(--clack-border-strong)]"
                    : "border-[color:var(--clack-border-subtle)]",
                  "focus-within:border-[color:var(--clack-border-accent)] focus-within:shadow-[var(--clack-glow-accent)]",
                )}
              >
                <Textarea
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <button
                        ...
                        className={cn(
                          "clack-toolbar-button inline-flex size-6 cursor-pointer items-center justify-center",
              ...
              <div className="flex min-w-0 items-center gap-1.5 text-[10.5px] text-[var(--clack-text-3)]">
                <span
                  className={cn(
                    "size-1.5 shrink-0 rounded-full transition-colors",
                    canCommit
                      ? "bg-[var(--clack-success)]"
                      : stagedCount > 0
                        ? "bg-[var(--clack-warning)]"
                        : "bg-[var(--clack-text-3)]",
                  )}
                />
                <span className="truncate font-medium text-[var(--clack-text-2)]">
                  {stagedCount === 0
                    ? "Nothing staged"
                    : `${stagedCount} ${stagedCount === 1 ? "file" : "files"} staged`}
                </span>
                <span className="ml-auto shrink-0 truncate text-[var(--clack-text-3)]">
                  {pushStatusLabel}
                </span>
              </div>
            ...
            {scm.allClean ? (
              ...
              <div
                ...
                className="relative min-h-0 flex-1 outline-none focus-visible:ring-1 focus-visible:ring-[var(--clack-focus)]"
              >
                <div
                  ref={scrollRef}
...
function PanelCenter({
  ...
  return (
    <div className="flex flex-1 flex-col items-center justify-center gap-2 px-6 text-center">
      <div className="text-sm font-semibold text-[var(--clack-text-1)]">
        {title}
      </div>
      {body ? (
        <div className="max-w-64 text-[11px] leading-relaxed text-[var(--clack-text-3)]">
          {body}
        </div>
      ) : null}
      {action}
...
function CleanTreeHint({ repoLabel }: { repoLabel: string }) {
  return (
    <div className="flex min-h-0 flex-1 flex-col items-center justify-center gap-1.5 px-4 text-center">
      <div className="clack-card flex size-8 items-center justify-center text-[var(--clack-success)]">
        <HugeiconsIcon
      ...
      <div className="text-[12px] font-semibold text-[var(--clack-text-1)]">
        Working tree clean
      </div>
      <div className="text-[10.5px] leading-snug text-[var(--clack-text-3)]">
        on <span className="font-mono text-[var(--clack-text-2)]">{repoLabel}</span>
      </div>
    </div>
  );
}
...
function DivergedBanner() {
  return (
    <div className="mx-2 mt-1 flex h-7 items-center gap-1.5 rounded-[var(--clack-radius-button)] border border-[color:var(--clack-border-strong)] bg-[var(--clack-surface-2)] px-2 text-[10.5px] leading-none text-[var(--clack-text-3)]">
      <HugeiconsIcon
        icon={Alert02Icon}
        size={11}
        strokeWidth={1.9}
...
function ListHeader({
  ...
  return (
    <div className="flex h-7 items-center gap-2 px-3">
      ...
      <span className="inline-flex h-4 min-w-4 items-center justify-center rounded-full border border-[color:var(--clack-border-subtle)] px-1 text-[9.5px] font-semibold tabular-nums text-[var(--clack-text-3)]">
        {row.count}
      </span>
      <label className="ml-auto flex shrink-0 cursor-pointer select-none items-center gap-1.5 text-[10.5px] font-medium text-[var(--clack-text-3)] hover:text-[var(--clack-text-1)]">
        <span>All</span>
        <Checkbox
    ...
    <ContextMenu>
      <ContextMenuTrigger asChild>
        <div
          ...
          className={cn(
            "group relative flex h-[30px] items-center gap-2 rounded-[var(--clack-radius-button)] pl-2 pr-2 transition-all duration-100",
            focused
              ? "bg-[var(--clack-accent-soft)] ring-1 ring-inset ring-[var(--clack-border-accent)]"
              : isSelected
                ? "bg-[var(--clack-accent-soft)] text-[var(--clack-text-1)]"
                : "hover:bg-[rgba(159,177,210,0.07)]",
          )}
        >
          <span
            className={cn(
            ...
            <div className="flex min-w-0 flex-1 items-baseline gap-1.5 leading-none">
              <span
                className={cn(
                  ...
                  isSelected || focused
                    ? "font-semibold text-[var(--clack-text-1)]"
                    : "font-medium text-[var(--clack-text-2)]",
              ...
              {pathLabel ? (
                <span className="min-w-0 flex-1 truncate text-[10.5px] leading-tight text-[var(--clack-text-3)]">
                  {pathLabel}
                </span>
              ) : null}
            </div>
    ...
    <Tooltip>
      <TooltipTrigger asChild>
        <Button
          size="icon-sm"
          variant="toolbar"
          className="size-6 cursor-pointer p-3 disabled:cursor-not-allowed"
          aria-label={label}
          disabled={disabled}
          onClick={onClick}
        >
...
function CommitFeedback({
  ...
  return (
    <div
      className={cn(
        "pointer-events-none absolute inset-x-3 top-[calc(100%-0.25rem)] z-20 flex min-w-0 items-center gap-1.5 rounded-[var(--clack-radius-panel)] border px-2.5 py-1.5 text-[11px] leading-snug shadow-lg shadow-black/15 backdrop-blur transition-all duration-200",
        isVisible ? "translate-y-0 opacity-100" : "-translate-y-1 opacity-0",
        isError
          ? "border-destructive/30 bg-[var(--clack-surface-raised)] text-destructive"
          : "border-[color:var(--clack-border-strong)] bg-[var(--clack-surface-raised)] text-[var(--clack-text-3)]",
      )}
    >
      <span
        className={cn(
          "size-1.5 shrink-0 rounded-full",
          isError ? "bg-destructive" : "bg-[var(--clack-success)]",
        )}
      />
      <span
        className={cn(
          "min-w-0 flex-1 truncate",
          isError ? "text-destructive" : "text-[var(--clack-text-3)]",
        )}
      >
