...
export type PaneRemoval = {
  tree: PaneNode | null;
  activeLeafId: PaneId | null;
  removed: boolean;
};

export function removeLeafAndFocus(
  tree: PaneNode,
  targetId: PaneId,
  activeLeafId: PaneId,
): PaneRemoval {
  if (!hasLeaf(tree, targetId)) {
    return { tree, activeLeafId, removed: false };
  }

  const nextTree = removeLeaf(tree, targetId);
  if (nextTree === null) {
    return { tree: null, activeLeafId: null, removed: true };
  }

  if (activeLeafId !== targetId) {
    return { tree: nextTree, activeLeafId, removed: true };
  }

  const remaining = leafIds(nextTree);
  const sibling = siblingLeafOf(tree, targetId);
  return {
    tree: nextTree,
    activeLeafId:
      sibling !== null && remaining.includes(sibling) ? sibling : remaining[0],
    removed: true,
  };
}
