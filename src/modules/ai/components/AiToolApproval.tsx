...
function AiToolApprovalImpl({ part, toolName, onRespond }: Props) {
  ...
  return (
    <div className="clack-ai-block overflow-hidden shadow-sm">
      <div className="flex items-center gap-2 border-b border-[color:var(--clack-border-subtle)] px-3 py-2">
        <span className="size-1.5 shrink-0 animate-pulse rounded-full bg-[var(--clack-warning)]" />
        <HugeiconsIcon
          icon={Icon}
          size={13}
          strokeWidth={1.75}
      ...
      <div className="flex items-center justify-end gap-1.5 border-t border-[color:var(--clack-border-subtle)] px-3 py-2">
        <Button
          size="sm"
          variant="destructive"
...
function PreviewBlock({
  ...
  if (toolName === "bash_run" || toolName === "bash_background") {
    ...
    return (
      <div className="space-y-1.5">
        ...
        <pre
          className={cn(
            "max-h-40 overflow-auto rounded-[var(--clack-radius-button)] bg-[var(--clack-surface-2)] p-2 font-mono text-[11px] leading-relaxed text-[var(--clack-text-2)]",
          )}
        >
  ...
  if (toolName === "write_file") {
    ...
    return (
      <div className="space-y-0.5 font-mono text-[11px]">
        <div className="text-[var(--clack-text-3)]">{String(input.path ?? "")}</div>
  ...
  if (toolName === "edit") {
    ...
    return (
      <div className="space-y-0.5 font-mono text-[11px]">
        <div className="text-[var(--clack-text-3)]">
          {String(input.path ?? "")}
  ...
  if (toolName === "create_directory") {
    return (
      <div className="font-mono text-[11px] text-[var(--clack-text-3)]">
        {String(input.path ?? "")}
      </div>
    );
  }
  return (
    <pre className="overflow-auto rounded-[var(--clack-radius-button)] bg-[var(--clack-surface-2)] p-2 font-mono text-[11px] leading-relaxed text-[var(--clack-text-2)]">
