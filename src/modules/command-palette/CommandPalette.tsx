...
export function CommandPalette({
  ...
  useEffect(() => {
    ...
    const handle = window.setTimeout(() => {
      document.getElementById("clack-command-palette-input")?.focus();
    }, 0);
  ...
  return (
    ...
    >
      ...
      >
        <CommandInput
          id="clack-command-palette-input"
          value={query}
                  ...
                  >
                    ...
                    {t.id === themeId ? (
                      <HugeiconsIcon
                        ...
                        size={14}
                        strokeWidth={2}
                          className="ml-auto text-[var(--clack-text-3)]"
                      />
                    ) : null}
                  </CommandItem>
                ))}
                  ...
                  >
                    {content.results.map((hit) => (
                      ...
                      >
                        ...
                        <span className="ml-auto max-w-64 shrink-0 truncate text-[11px] font-normal text-[var(--clack-text-3)]">
                          {hit.rel}:{hit.line}
                        </span>
                      </CommandItem>
                    ))}
              ...
              <CommandGroup heading="Search modes">
                {MODE_HINTS.map((hint) => (
                  <CommandItem
                    ...
                    className="text-[12.5px]"
                  >
                    <kbd className="rounded-[var(--clack-radius-button)] bg-[var(--clack-surface-2)] px-1.5 py-0.5 font-mono text-[11px] text-[var(--clack-text-2)]">
                      {hint.sigil}
                    </kbd>
...
function StatusItem({
  ...
  return (
    ...
    >
      ...
      <span
        className={tone === "error" ? "text-destructive" : "text-[var(--clack-text-3)]"}
      >
        {label}
      </span>
    </CommandItem>
...
function EmptyHint() {
  return (
    <div className="flex flex-col items-center gap-2 px-4 py-10 text-center text-sm text-[var(--clack-text-3)]">
      <div className="clack-card flex size-9 items-center justify-center text-[var(--clack-accent)]">
        <HugeiconsIcon icon={CommandIcon} size={18} strokeWidth={1.5} />
      </div>
      <span>No Clack commands found. Type ? to see search modes.</span>
    </div>
  );
}
